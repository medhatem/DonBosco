...
public class Login extends HttpServlet
{
			...
			throws ServletException, IOException
	{
		...
		if (nomU != null && motP != null)
		{
			...
			j = equipeSoccerInterrogation.getGestionJoueur().login(nomU, motP);

			
			if(j == null) {
				session.setAttribute("TypeErreur", "Erreur");
		        session.setAttribute("listeMessageErreur", "Utilisateur ou mot de passe incorrect.");
		        session.setAttribute("typeUtilisateur", null);
				...
				if(j.JoueurEstDeType()==EquipeSoccerConstantes.SPECTATEUR) {
			        ...
			        session.setAttribute("listeMessageErreur", "Nom d'utilisateur ou mot de passe incorrect.");
					RequestDispatcher dispatcher = request.getRequestDispatcher("/index.jsp");
					dispatcher.forward(request, response);
				}
